...
export default {
  data() {
    ...
    const clientNameUniqueValidator = (rule, value, callback) => {
      if (value === '') {
        callback();
      } else {
        UserService.checkUserUnique(value)
          .then((res) => {
            if (res.errorCode === 0) {
              callback();
    ...
    return {
      ...
      rules: {
        clientName: [
          ...
          { validator: clientNameUniqueValidator, trigger: ['blur'] },
        ],
        clientPassword: [
          { required: true, message: '必填项', trigger: ['blur', 'change'] },
        ],
  ...
  methods: {
    handleSubmit() {
      UserService.addUser(this.formModel).then((res) => {
        if (res.errorCode === 0) {
          this.$message({
            type: 'success',
            message: '添加成功！',
