<template>
  ...
  <el-container class="content__body">
    <el-header class="heading">子系统管理</el-header>
    <el-main class="content">
      ...
      <div class="ope clearfix">
        ...
        <!-- 添加 -->
        <el-button type="success" size="small" @click="handleAdd()">添加子系统</el-button>
        <!-- /添加 -->

        <!-- 搜索 -->
        <el-form :inline="true" class="search-group">
          <el-form-item>
            <el-input placeholder="请输入关键字" size="small" v-model="SystemName">
              <template slot="prepend">子系统名称</template>
            </el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-table :data="tableData" v-loading="loading" class="datagrid_table">
        ...
        <el-table-column label="系统名称" width="160">
          <template slot-scope="scope">{{ scope.row.SystemName }}</template>
        </el-table-column>

        <el-table-column label="系统编号" width="160">
          <template slot-scope="scope">{{ scope.row.SystemIdenty }}</template>
        </el-table-column>

        <el-table-column label="系统级别" width="160">
          <template slot-scope="scope">{{ scope.row.SystemLevel }}</template>
        </el-table-column>

        <el-table-column label="系统所属行政区划" width="160">
          <template slot-scope="scope">{{ scope.row.UnitCode }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <a class="ope-link" href="#"
               @click.prevent="handleDetail(scope.row.SystemId)">详情</a>
            <a class="ope-link" href="#"
               @click.prevent="handleEdit(scope.row.SystemId)">编辑</a>
            <a class="ope-link" href="#"
               @click.prevent="handleDelete(scope.row.SystemId)">删除</a>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    reload(isInitCurrentPage = false) {
      ...
      SubsysService.getList( { SystemName: this.SystemName }, { pageSize, currentPage }).then((res) => {
        if (res.errorCode === 0) {
          const {
            result: {
              items,
    ...
    handleDelete(SystemId) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(() => SubsysService.delete({ SystemId }))
          .then((res) => {
            if (res.errorCode === 0) {
              this.$message({
                type: 'success',
